    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 z-0 overflow-hidden">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1200px] h-[1000px] bg-[#0088ff]/20 blur-[200px] pointer-events-none opacity-80" />
                <div className="absolute -top-[10%] -left-[10%] w-[700px] h-[700px] bg-[#00fbff]/15 blur-[150px] pointer-events-none rounded-full" />
                <div className="absolute -bottom-[20%] -right-[10%] w-[800px] h-[800px] bg-[#0044cc]/20 blur-[180px] pointer-events-none rounded-full" />
            </div>
            ...
            <div className="container mx-auto px-6 lg:px-12 pt-12 pb-16 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center relative z-20">
                    ...
                    <div className="space-y-0 leading-none">
                        {['We Build', 'High-Performance', 'Digital Brands'].map((line, i) => (
                            <div key={i} className="overflow-hidden">
                                <h1
                                    className="h-word inline-block font-display text-4xl md:text-5xl lg:text-7xl font-black uppercase leading-[0.92] tracking-tighter"
                                    style={i === 1 ? {
                ...
                <div className="relative flex items-center justify-center">
                    ...
                    >
                        <div className="rounded-[2.5rem] p-8 overflow-hidden backdrop-blur-3xl" style={{
                            ...
                            <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/10">
                                <div>
                                    <p className="font-body text-[10px] font-black uppercase tracking-[0.3em] text-[#00fbff]">Live Data</p>
                                    <p className="font-body text-sm text-white font-bold mt-1">Growth Index</p>
                                </div>
                            ...
                            <div className="mb-6 inline-flex items-center gap-2.5 px-4 py-2 rounded-xl bg-[#0088ff]/15 border border-[#0088ff]/30">
                                <TrendingUp className="w-4 h-4 text-[#00fbff]" />
                                <span className="font-body font-black text-sm text-[#00fbff]">+4.5x Growth</span>
                            </div>

                            <div className="grid grid-cols-3 gap-3 mb-6">
                                {[['ROAS', '8.4x'], ['AOV', '£164'], ['Conv.', '5.2%']].map(([l, v]) => (
                                    <div key={l} className="p-3.5 rounded-2xl bg-white/[0.05] border border-white/10">
                                        <p className="font-body text-[9px] font-black uppercase tracking-widest text-[#00d4ff] mb-1.5">{l}</p>
                                        <p className="font-body text-lg font-black text-white">{v}</p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/10">
                                <div className="flex items-end gap-1.5 h-16">
                                    {[25, 50, 35, 70, 45, 85, 55, 95, 65, 100].map((h, i) => (
                                        <motion.div key={i}
                                            className="flex-1 rounded-sm"
                                            style={{
                                                height: `${h}%`,
                                                originY: 1,
                                                background: i >= 8 ? 'linear-gradient(to top, #0044cc, #00fbff)' : 'rgba(0,136,255,0.2)',
                                            }}
                                            initial={{ scaleY: 0 }}
                                            animate={{ scaleY: 1 }}
